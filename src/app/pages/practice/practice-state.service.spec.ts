...
describe('PracticeState', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.inject(StorageService).saveSettings({ apiKey: 'key', modelPriority: ['m1'], theme: 'dark' });
  });

  it('APIキー未設定なら送信せずエラー通知を出す', async () => {
    TestBed.inject(StorageService).saveSettings({ apiKey: '', modelPriority: ['m1'], theme: 'dark' });
  ...
  it('成功時に結果を保存し入力欄をクリアする', async () => {
    ...
    expect(TestBed.inject(StorageService).sessions().length).toBe(1);
  });
  ...
  it('submitBulk で複数件を並列添削しstorageへ保存する', async () => {
    ...
    expect(TestBed.inject(StorageService).sessions().length).toBe(2);
