/**
 * @file Gemini レスポンスの `<tag>...</tag>` ブロックを抽出する低レベルヘルパー。
 * 構造化データ用の extractTaggedJson（タグ抽出 → コードフェンス除去 → JSON.parse → 型検証）と、
 * 自由記述の英文・Markdown用の extractTaggedText（タグ抽出のみ、JSON化しない）の2種類を提供する。
 * レスポンス各セクションのスキーマ検証（何を有効な mistakes/evaluation とみなすか）は
 * gemini-response.util.ts の責務であり、このファイルはタグ単位の抽出プリミティブに徹する。
 * gemini.service.ts は全セクション（解説5項目 + corrected-text/levelup-text/mistakes/evaluation/
 * levelup/review）をこれらで個別に抽出しており、1タグの抽出失敗はそのフィールドが undefined に
 * なるだけで他タグの抽出には影響しない。
 */
