 ...
 * レスポンス解析は2段構えで、タグ単位の抽出プリミティブは gemini-parse.util.ts、各セクションの
 * スキーマ検証（parseMistakes/parseEvaluation/parseLevelUp/parseReview）と解説5項目の定義
 * （PROSE_SECTIONS）は gemini-response.util.ts が持つ。このサービス自身は通信・モデルフォールバック・
 * 進捗通知・ロギングに専念する。
...
import { extractTaggedText, ParseFailureStage } from '@core/gemini/gemini-parse.util';
import {
  buildLegacyProse,
  parseEvaluation,
  parseLevelUp,
  ...
  parseReview,
  PROSE_SECTIONS,
} from '@core/gemini/gemini-response.util';
...
export class GeminiService {
  ...
  private async callApi(
    ...
    const mistakes = parseMistakes(text, warn('mistakes'));
    const evaluation = parseEvaluation(text, warn('evaluation'));
    const levelUpItems = parseLevelUp(text, warn('levelup'));
    ...
    const reviewItems = parseReview(text, warn('review'));
    ...
    const corrected = buildLegacyProse(
      PROSE_SECTIONS.map((s) => ({ heading: s.heading, text: prose[s.id] })),
    );
    const correctedEn = buildLegacyProse(
